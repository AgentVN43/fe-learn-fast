...
export default function StudySetDetailPage() {
  ...
  // Get progress data
  const { data: progressDataResponse } = useProgress(studySetId);
  ...
  // Check auth before entering learn/test mode
  ...
  if (viewMode === "learn") {
    const cardsToUse =
      shuffledFlashcards.length > 0 ? shuffledFlashcards : learnCards;
  ...
  if (viewMode === "review") {
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-100">
          <div className="bg-white rounded-lg shadow-md p-6 max-w-md text-center">
            <p className="text-gray-600 mb-4">
              ✅ Không có thẻ cần ôn tập lúc này
            </p>
            <button
              onClick={handleBackToDetail}
              className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg transition"
    ...
    const reviewFlashcards: Flashcard[] = (quickReviewCards as any[]).map(
      (progress) => ({
  ...
  // Handle test click with auth check
  ...
  return (
    <>
      <LoginModal
        ...
        message="Vui lòng đăng nhập để bắt đầu học hoặc làm bài kiểm tra"
      />
      <StudySetDetail
        studySet={studySet}
        flashcards={learnCards}
